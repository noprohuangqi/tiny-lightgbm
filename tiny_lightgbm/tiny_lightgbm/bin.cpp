...
namespace Tiny_LightGBM {
	...
	void BinMapper::FindBin(double* values, int num_values, int num_row) {
		...
		// 如果没有正常值，或者第一个值是正数
		//把0元素push进去，注意distinct_values是包含所有数据的，包括三类
		...
		//注意tiny-lightgbm只实现了numerical的数据分类，即所有的feature都是连续的，而不是类别的。
		...
		//这里的num_bin_也是一个核心参数，后续会有很多应用。
		//简单地讲，num_bin_是bin_upper_bound_的size
		...
		//default_bin_也是BinMapper的一个核心参数
		//意义是，如果某个feature要push一个数据0进去，那么会push到哪个bin的位置上去。
		//valuetobin一个简单的二分查找
